...
export function parseFare(fareString) {
  // Your code here
  if (typeof fareString !== 'string' || parseFloat(fareString) === NaN || fareString === "" || isNaN(fareString)) {
    return -1
  }
...
export function calculateSurge(baseFare, surgeMultiplier) {
  // Your code here
  if (baseFare <= 0 || surgeMultiplier <= 0 || typeof surgeMultiplier !== 'number' || typeof baseFare !== 'number') {
    return 0
  }
...
export function findCheapestAndCostliest(...fares) {
  // Your code here
  let arr = fares.filter((fare) => typeof fare === 'number')
  ...
  let rtn = {
    cheapest: Math.min(...arr),
    costliest: Math.max(...arr)
  }
